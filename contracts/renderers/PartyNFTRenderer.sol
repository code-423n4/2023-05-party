...
contract PartyNFTRenderer is RendererBase {
    ...
    function generateName(uint256 tokenId) private view returns (string memory) {
        ...
        if (totalVotingPower == 0) {
            return string.concat("Party Card #", tokenId.toString());
        } else {
    ...
    function generateDescription(
        ...
        if (totalVotingPower == 0) {
            return
                string.concat(
                    "This Party Card represents membership in ",
                    partyName,
                    ". Head to ",
                    generateExternalURL(),
    ...
    function generateVotingPowerPercentage(uint256 tokenId) private view returns (string memory) {
        ...
        if (intrinsicVotingPowerPercentage == 0) {
            return "--";
        } else if (intrinsicVotingPowerPercentage == 1e18) {
            return "100";

...
contract PartyGovernanceNFTTest is Test, TestUtils {
    ...
    function testBurn_withApproval() external {
        (Party party, , ) = partyAdmin.createParty(
            partyImpl,
            PartyAdmin.PartyCreationMinimalOptions({
                host1: address(this),
        ...
        // Approve another address to burn the token
        address approvedAddress = _randomAddress();
        vm.prank(recipient);
        party.approve(approvedAddress, tokenId);

        // Burn token with approved address
    ...
    function testBurn_revertsIfNotCalledByOwnerOrApproved() external {
        (Party party, , ) = partyAdmin.createParty(
            partyImpl,
            PartyAdmin.PartyCreationMinimalOptions({
                host1: address(this),
        ...
        // Attempt to burn token without being owner or approved
        address notApproved = _randomAddress();
        vm.prank(notApproved);
        vm.expectRevert(PartyGovernanceNFT.UnauthorizedToBurnError.selector);
        party.burn(tokenId);
    }

    function testBurn_revertsIfCalledByOwnerBeforePartyStarted() external {
        (Party party, , ) = partyAdmin.createParty(
            partyImpl,
            PartyAdmin.PartyCreationMinimalOptions({
                host1: address(this),
        ...
        address recipient = _randomAddress();
        vm.prank(address(partyAdmin));
        uint256 tokenId = party.mint(recipient, 1, recipient);
    ...
    function testBurn_cannotBurnAndAcceptInSameBlock() external {
        (Party party, , ) = partyAdmin.createParty(
            partyImpl,
            PartyAdmin.PartyCreationMinimalOptions({
                host1: address(this),
    ...
    function testRageQuit() external {
        (Party party, , ) = partyAdmin.createParty(
            partyImpl,
            PartyAdmin.PartyCreationMinimalOptions({
                host1: address(this),
        ...
        vm.prank(recipient);
        party.rageQuit(tokenIds, tokens, recipient);
    ...
    function testRageQuit_multiple() external {
        ...
        for (uint256 i; i < members.length; ++i) {
            ...
            vm.prank(member);
            party.rageQuit(tokenIds, tokens, member);
